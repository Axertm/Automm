...
describe('DealWizardStore', () => {
  it('starts at step 1 with nothing confirmed', () => {
    ...
    expect(state.participantConfirmed).toBe(false);
    expect(state.buyerId).toBeNull();
    expect(state.sellerId).toBeNull();
  ...
  it('creates, retrieves, and updates a wizard session keyed by channel', () => {
    ...
    const updated = store.update('channel-1', {
    ...
    expect(store.get('channel-1')?.participantId).toBe('participant-1');
  });
