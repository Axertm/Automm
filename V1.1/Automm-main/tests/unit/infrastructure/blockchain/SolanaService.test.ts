...
function makeService() {
  ...
  const service = new SolanaService(rpcClient, silentLogger);
  return { service, rpcClient };
}

describe('SolanaService', () => {
  it('generates a wallet with a valid base58 SOL address and a decodable secret key', async () => {
    ...
    expect(decoded).toHaveLength(64); // ed25519 secret key length
  });
