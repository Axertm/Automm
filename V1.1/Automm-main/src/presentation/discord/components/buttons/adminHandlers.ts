...
const retryPayoutConfirm: ButtonHandler = async (interaction, decoded, deps) => {
  await interaction.update({ embeds: [buildSimpleEmbed('⏳ Retrying payout…')], components: [] });
  const result = await deps.adminRetryPayout.execute(asDealId(decoded.dealId), interaction.user.id);
  await interaction.editReply({
    embeds: [
      result.ok
        ? buildSimpleEmbed('✅ Payout re-broadcast.', 'success')
        : buildSimpleEmbed(`❌ ${result.error.message}`, 'error'),
    ],
  });
};
...
export function registerAdminHandlers(registry: HandlerRegistry): void {
  ...
  registry.registerButton('admin:confirm_retry_payout', retryPayoutConfirm);
  registry.registerButton('admin:cancel_retry_payout', handleCancel);
}
