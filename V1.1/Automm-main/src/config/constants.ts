...
// scanning needs to run. Kept well above LTC_PROVIDER_RATE_LIMITS.blockcypher's
// 200/hour ceiling with headroom for concurrent payouts and deposit scanning
// sharing the same quota — note that ceiling is a real server-side limit, not
// one enforced locally (RequestQueue's TokenBucketLimiter only throttles the
// per-second rate), so nothing stops this loop from tripping actual 429s if
// pushed too fast.
