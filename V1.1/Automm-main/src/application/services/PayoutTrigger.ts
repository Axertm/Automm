...
import type { ConfirmPayoutTransactionsUseCase } from '../use-cases/scanning/ConfirmPayoutTransactionsUseCase.js';
...
export class PayoutTrigger {
  ...
  async execute(deal: Deal, actorId: string, action: string): Promise<Result<Deal, DomainError | Error>> {
    return this.mutex.runExclusive(deal.id, async () => {
      ...
      // Best-effort: some chains (e.g. Solana, already broadcast at
      // 'confirmed' commitment) can reach the required confirmation level
      // within seconds — checking right away can complete the deal
      // immediately instead of waiting for the next scheduled poll.
      await this.confirmPayoutTransactionsUseCase?.execute(deal.id).catch(() => undefined);
