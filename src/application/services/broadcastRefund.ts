...
/**
 * The shared, money-moving core of every refund path — the admin `/refund`
 * command and the seller-initiated refund button both funnel through here so
 * the duplicate-broadcast protection lives in exactly one place. It sends the
 * deal's confirmed deposited balance (minus the network fee) back to
 * `refundAddress`; no escrow fee is charged on a refund.
 *
