...
export function buildFinalSummaryMessage(state: DealWizardState, deps: AppDependencies) {
  ...
  embed.addFields(
    ...
    { name: 'Escrow Fee', value: `${(feeBps / 100).toFixed(2)}%`, inline: true },
    {
      ...
      value:
        state.currency === 'LTC'
          ? String(deps.env.LTC_REQUIRED_CONFIRMATIONS)
          : state.currency === 'USDT'
            ? `${deps.env.USDT_REQUIRED_CONFIRMATIONS} blocks (Polygon)`
            : 'Finalized (Solana)',
      inline: true,
    },
    {
...
async function handleConfirm(interaction: ButtonInteraction, deps: AppDependencies): Promise<void> {
  ...
  if (updated.buyerFinalConfirmed && updated.sellerFinalConfirmed) {
    ...
    // The "Finalizing deal…" placeholder is never edited again — finalizeDeal
    // only ever sends brand-new messages (or a followUp on failure) — so it
    // would otherwise sit there forever looking stuck. Remove it now that
    // real content (status/wallet embeds, or an error) has been posted.
