...
import { PrismaBackupAccountRepository } from './infrastructure/persistence/PrismaBackupAccountRepository.js';
import { PrismaDealBackupRepository } from './infrastructure/persistence/PrismaDealBackupRepository.js';
import { PrismaSavedPayoutAddressRepository } from './infrastructure/persistence/PrismaSavedPayoutAddressRepository.js';
...
import { TatumProvider } from './infrastructure/blockchain/litecoin/providers/TatumProvider.js';
...
import { PolygonRpcClient } from './infrastructure/blockchain/usdt-polygon/PolygonRpcClient.js';
import { UsdtPolygonService } from './infrastructure/blockchain/usdt-polygon/UsdtPolygonService.js';
...
export async function bootstrap(): Promise<App> {
  ...
  const backupAccountRepository = new PrismaBackupAccountRepository(prisma);
  const dealBackupRepository = new PrismaDealBackupRepository(prisma);
  const savedPayoutAddressRepository = new PrismaSavedPayoutAddressRepository(prisma);
  ...
  // Both slots point at the same Tatum account — FailoverUtxoProvider still
  // gives an automatic second attempt through its own retry/backoff cycle if
  // the first errors out, and its dual-broadcast still fires twice (harmless
  // and idempotent; see FailoverUtxoProvider.broadcastRawTransaction).
  const utxoProvider = new FailoverUtxoProvider(
    new TatumProvider(env.TATUM_API_KEY),
    new TatumProvider(env.TATUM_API_KEY),
    ltcLogger,
  );
  ...
  const usdtLogger = logger.child({ component: 'usdt-polygon' });
  const polygonRpcClient = new PolygonRpcClient(
    env.POLYGON_RPC_URL,
    env.POLYGON_RPC_URL_FALLBACK || undefined,
    usdtLogger,
  );
  const usdtService = new UsdtPolygonService(
    polygonRpcClient,
    env.USDT_CONTRACT_ADDRESS,
    usdtLogger,
    env.USDT_REQUIRED_CONFIRMATIONS,
  );

  const blockchainServiceFactory = new BlockchainServiceFactory(ltcService, solService, usdtService);
  ...
  const requestRelease = new RequestReleaseUseCase(
    dealRepository,
    notifierProxy,
    auditRecorder,
  ...
  const confirmRelease = new ConfirmReleaseUseCase(
    dealRepository,
    notifierProxy,
    auditRecorder,
    dealBackupRepository,
