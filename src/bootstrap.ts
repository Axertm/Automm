...
import { AesGcmEncryptionService } from './infrastructure/security/AesGcmEncryptionService.js';
...
export async function bootstrap(): Promise<App> {
  ...
  const encryptionService = new AesGcmEncryptionService(env.ENCRYPTION_MASTER_KEY);
  ...
  const requestRefund = new RequestRefundUseCase(dealRepository, notifierProxy, auditRecorder);
