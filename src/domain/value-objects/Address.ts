...
function isValidEvmAddress(address: string): boolean {
  try {
    // isEvmAddress accepts both checksummed and all-lowercase hex — either
    // is fine here since ethers normalizes to checksum form before signing.
...
export function isValidAddress(currency: Currency, address: string): boolean {
  ...
  if (currency === 'LTC') return isValidLitecoinAddress(address);
  if (currency === 'SOL') return isValidSolanaAddress(address);
