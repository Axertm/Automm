...
import { PrismaBackupAccountRepository } from '../../../src/infrastructure/persistence/PrismaBackupAccountRepository.js';
import { PrismaDealBackupRepository } from '../../../src/infrastructure/persistence/PrismaDealBackupRepository.js';
import { PrismaSavedPayoutAddressRepository } from '../../../src/infrastructure/persistence/PrismaSavedPayoutAddressRepository.js';
...
afterEach(async () => {
  ...
  await prisma.dealBackup.deleteMany();
  await prisma.wallet.deleteMany();
  await prisma.deal.deleteMany();
  await prisma.backupAccount.deleteMany();
  await prisma.savedPayoutAddress.deleteMany();
});
...
describe('PrismaBackupAccountRepository', () => {
  it('adds, lists, and is idempotent on a duplicate add', async () => {
    const repo = new PrismaBackupAccountRepository(prisma);
    await repo.add('owner-1', 'backup-1');
    await repo.add('owner-1', 'backup-1');
    await repo.add('owner-1', 'backup-2');

    const backups = await repo.findByOwner('owner-1');
    expect(backups.map((b) => b.backupDiscordId).sort()).toEqual(['backup-1', 'backup-2']);
  });

  it('finds owners by backup account (reverse lookup)', async () => {
    const repo = new PrismaBackupAccountRepository(prisma);
    await repo.add('owner-1', 'shared-backup');
    await repo.add('owner-2', 'shared-backup');

    const owners = await repo.findByBackup('shared-backup');
    expect(owners.map((b) => b.ownerDiscordId).sort()).toEqual(['owner-1', 'owner-2']);
  });

  it('removes a specific owner/backup pair without affecting others', async () => {
    const repo = new PrismaBackupAccountRepository(prisma);
    await repo.add('owner-1', 'backup-1');
    await repo.add('owner-1', 'backup-2');
    await repo.remove('owner-1', 'backup-1');

    const backups = await repo.findByOwner('owner-1');
    expect(backups.map((b) => b.backupDiscordId)).toEqual(['backup-2']);
  });
});

describe('PrismaDealBackupRepository', () => {
  it('upserts on (dealId, role) — activating a new backup replaces the previous one for that role', async () => {
    const dealRepo = new PrismaDealRepository(prisma);
    const backupRepo = new PrismaDealBackupRepository(prisma);
    const deal = Deal.create(makeDealProps());
    await dealRepo.save(deal);

    await backupRepo.upsert({ dealId: deal.id, role: 'BUYER', backupDiscordId: 'backup-a', addedByAdminId: 'admin-1' });
    await backupRepo.upsert({ dealId: deal.id, role: 'BUYER', backupDiscordId: 'backup-b', addedByAdminId: 'admin-2' });
    await backupRepo.upsert({ dealId: deal.id, role: 'SELLER', backupDiscordId: 'backup-c', addedByAdminId: 'admin-1' });

    const records = await backupRepo.findByDealId(deal.id);
    expect(records).toHaveLength(2);
    expect(records.find((r) => r.role === 'BUYER')?.backupDiscordId).toBe('backup-b');
    expect(records.find((r) => r.role === 'SELLER')?.backupDiscordId).toBe('backup-c');
  });

  it('removes a role-specific backup', async () => {
    const dealRepo = new PrismaDealRepository(prisma);
    const backupRepo = new PrismaDealBackupRepository(prisma);
    const deal = Deal.create(makeDealProps());
    await dealRepo.save(deal);
    ...
    await backupRepo.upsert({ dealId: deal.id, role: 'BUYER', backupDiscordId: 'backup-a', addedByAdminId: 'admin-1' });
    await backupRepo.remove(deal.id, 'BUYER');

    expect(await backupRepo.findByDealId(deal.id)).toHaveLength(0);
  });
});

describe('PrismaSavedPayoutAddressRepository', () => {
  it('saves and finds a per-currency address, and overwrites on re-save', async () => {
    const repo = new PrismaSavedPayoutAddressRepository(prisma);
    expect(await repo.find('user-1', 'LTC')).toBeNull();

    await repo.save('user-1', 'LTC', 'ltc-address-1');
    expect(await repo.find('user-1', 'LTC')).toBe('ltc-address-1');
    expect(await repo.find('user-1', 'SOL')).toBeNull();

    await repo.save('user-1', 'LTC', 'ltc-address-2');
    expect(await repo.find('user-1', 'LTC')).toBe('ltc-address-2');
  });
});
