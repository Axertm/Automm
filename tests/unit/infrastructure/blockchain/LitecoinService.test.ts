...
function makeService() {
  ...
  const encryption = new AesGcmEncryptionService('a'.repeat(64));
  const service = new LitecoinService(utxoProvider, encryption, silentLogger);
  return { service, utxoProvider, encryption };
}

describe('LitecoinService', () => {
  it('generates a wallet with a valid bech32 LTC address and an encrypted, decryptable private key', async () => {
