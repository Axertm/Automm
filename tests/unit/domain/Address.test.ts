...
describe('Address', () => {
  ...
  it('validates a real checksummed EVM address for USDT', () => {
    expect(isValidAddress('USDT', '0xc2132D05D31c914a87C6611C10748AEb04B58e8F')).toBe(true);
  });

  it('validates an all-lowercase EVM address for USDT', () => {
    expect(isValidAddress('USDT', '0xc2132d05d31c914a87c6611c10748aeb04b58e8f')).toBe(true);
  });

  it('rejects garbage and non-EVM addresses as a USDT address', () => {
    expect(isValidAddress('USDT', 'not-an-address')).toBe(false);
    expect(isValidAddress('USDT', '0x1234')).toBe(false);
    expect(isValidAddress('USDT', '11111111111111111111111111111112')).toBe(false);
  });
});
