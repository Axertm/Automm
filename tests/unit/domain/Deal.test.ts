...
describe('Deal', () => {
  ...
  describe('freeze / unfreeze', () => {
    ...
    it('freezes a deal stuck in PAYOUT_IN_PROGRESS and restores it on unfreeze, so a stalled payout can be retried', () => {
      const deal = awaitingPayoutConfirmationDeal();
      deal.confirmPayoutAddressBySeller(SELLER);
      deal.startPayout();
      deal.freeze(ADMIN, 'payout broadcast stalled');
      expect(deal.state).toBe('FROZEN');
      expect(deal.frozenFromState).toBe('PAYOUT_IN_PROGRESS');
      deal.unfreeze();
      expect(deal.state).toBe('PAYOUT_IN_PROGRESS');
    });

    it('refuses to freeze a terminal deal', () => {
      const deal = fundedDeal();
      deal.refund(ADMIN, 'resolved', 'ltc1qrefundaddress');
